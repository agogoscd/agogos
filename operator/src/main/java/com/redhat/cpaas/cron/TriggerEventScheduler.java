    ...
    public void scheduleTimedTriggerEvent(Trigger trigger, String cron) throws SchedulerException {
        ...
        switch (trigger.getSpec().getTarget().getKind()) {
            case "Pipeline":
                ...
                break;
            case "Component":
                quartzJob = JobBuilder.newJob(ComponentBuildJob.class)
                        .withIdentity(trigger.getMetadata().getName(), trigger.getMetadata().getNamespace()) //
                        .usingJobData(ComponentBuildJob.COMPONENT_NAME, trigger.getSpec().getTarget().getName()) //
                        .usingJobData(ComponentBuildJob.COMPONENT_NAMESPACE, trigger.getMetadata().getNamespace()) //
                        .build();
                break;
