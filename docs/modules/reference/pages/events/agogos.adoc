...
2+s|xref:#componentbuild-events[Component build events]

|com.redhat.agogos.event.componentbuild.success.v1alpha1
|Component build finishes successfully.

|com.redhat.agogos.event.componentbuild.failure.v1alpha1
|Component build fails.

2+s|xref:#pipelinerun-events[Pipeline run events]

|com.redhat.agogos.event.pipelinerun.success.v1alpha1
|Pipeline run finishes successfully.

|com.redhat.agogos.event.pipelinerun.failure.v1alpha1
|Pipeline run fails.
|=== 
...
<2> The `build` key contains the Component build resource representation in the JSON format
...
Every Pipeline run event is sending the JSON representation of an Agogos
