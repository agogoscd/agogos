...
== Before you begin
This tutorial assumes you have already setup `minikube`, `kubectl` and
OpenJDK 17. See the following for installation instructions:

* link:https://minikube.sigs.k8s.io/docs/start/[minikube start]
* link:https://minikube.sigs.k8s.io/docs/drivers/kvm2/[kvm2 driver for minikube]
...
Next, write the following Components to a file and apply them with `kubectl`:
...
dependencies. Create a file with the following content and apply it to your
...
After a while, the Group Components and their dependants should be finished:
