...
This tutorial shows you how to run and use Agogos using Minikube.
...
we provide information about how to achieve when using Firewalld.
...
This command compiles all the Agogos packages. Then install the Agogos Operator, run:

[source,bash]
----
java -Dquarkus.profile=dev -jar cli/target/quarkus-app/quarkus-run.jar adm install --profile dev
----

This command installs Tekton, Knative Eventing and the Agogos CustomResourceDefinitions,
...
webhooks endpoint. These certificates are automatically used by Agogos.
...
After installing Agogos, you need to install a Builder in order to build stuff.
...
* The process is the same for any other Builder or Extensions, change the folder
...
This command creates service accounts, rolebindings, configuration maps, and copies
builder resources. Then change the namespace [command]`kubectl` uses by default:
