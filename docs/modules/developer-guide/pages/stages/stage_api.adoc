...
. Stored files should be small. This is not the place where artifacts built
  by the Builder should be saved.
. Refrain from storing binary files.
...
. What is the component (metadata)
. How to build it (source coordinates)
...
Results must be small (<4kB of data). If there is a requirement to store bigger
data, workspace should be used to do so and result should have a pointer to it.

=== Best practices

* If possible Build metadata should be stored within the system (usage of
  results and workspaces). Querying information about results should not
