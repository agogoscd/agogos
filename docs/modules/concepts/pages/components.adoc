...
. Optionally provide a list of pre and post
  xref:handlers.adoc[Handlers]

Component is an abstract concept which is not directly tied to any build tool
that should be used to build the source code. This is where
xref:builders.adoc[Builders]
...
A
xref:builds.adoc[Build]
object is created every time a new build of a Component is executed.
